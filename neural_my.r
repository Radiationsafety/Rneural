...
# Data preparation
getwd()
data <- read.csv("winemag-data_first150k.csv", header = TRUE)
str(data)
...
i=7
output <- compute(n, training[i,-3])
print(paste("net result = ",output$net.result, "| value from data file = ",training[i,3],sep=""))
res1 = output$net.result * ( max(data3$price) - min(data3$price) ) + min(data3$price)
res2 = training[i,3]  * ( max(data3$price) - min(data3$price) ) + min(data3$price)
print(paste("net result = ",
            format(res1,nsmall=1),
            "$ | value from data file = ",
            format(res2,nsmall=1),
            " $ . Ratio=",format(res1/res2*100,nsmall=0),"%",sep=""))
